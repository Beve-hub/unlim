...
interface UserData {
  id: number;
  Image_1: string; // Image URL as a string
  SKU: string;
  Name: string;
  Cost_Price: number;
  Quantity: number;
...
const FragranceNet = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        setLoading(true);
        try {
          const response = await axios.get<UserData[]>('http://3.88.1.181:8000/products/public/catalog?supplier=FragranceNet');
